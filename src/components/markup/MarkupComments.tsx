...
import { MarkupComment, ImageMarkup } from "@/types";
...
export default function MarkupComments({ 
  markupId, 
  ...
  const { user } = useAuth();
  const [comments, setComments] = useState<MarkupComment[]>([]);
  ...
  const loadComments = () => {
    ...
    const loadedComments: MarkupComment[] = stored ? JSON.parse(stored) : [];
    setComments(loadedComments);
    onCommentCountChange?.(loadedComments.length);
  };

  const saveComments = (updatedComments: MarkupComment[]) => {
    localStorage.setItem(`markup_comments_${markupId}`, JSON.stringify(updatedComments));
  ...
  const handleAddComment = async (parentId?: string, content?: string) => {
    ...
    try {
      ...
      console.log(`💬 마크업 댓글 추가: ${comment.content}`);
    } catch (error) {
  ...
  const handleResolveComment = (commentId: string) => {
    ...
    saveComments(updatedComments);
    console.log(`✅ 댓글 해결 상태 변경: ${commentId}`);
  };
